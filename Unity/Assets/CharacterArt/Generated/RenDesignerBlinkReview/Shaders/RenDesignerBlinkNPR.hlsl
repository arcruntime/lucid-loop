...
struct NprAttributes
{
    ...
    float2 uv : TEXCOORD0;
    float2 closedRG : TEXCOORD1;
    float2 closedBA : TEXCOORD2;
    half4 color : COLOR;
...
NprVaryings NprVertex(NprAttributes input)
{
    ...
    // Both endpoints are already linear; uniform interpolation commutes with raster interpolation.
    output.color = lerp(input.color, half4(input.closedRG, input.closedBA), saturate(_EyeBlinkWeight));

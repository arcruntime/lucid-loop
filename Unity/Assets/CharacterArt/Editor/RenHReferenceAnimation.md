...
Curated passed evidence is under `art/generated/characters/ren/video-reference-v1/unity-transport-review/`. Its source hashes describe the **pre-cap** controller and builder. Exact historical sources and metas were preserved at `.local/ren-h-reference-pre-cap-source/` before cap changes; the curated JSON was not rewritten to imply a rerun. `RenHReferenceVideoTransport.cs` and both smoke files are unchanged by the cap addition, and the decoder test has not been repeated.

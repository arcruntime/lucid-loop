...
namespace LucidLoop.Gyms
{
    ...
    public sealed class DialogueOrnament : MaskableGraphic
    {
        ...
        public Color Gold = new Color(.75f, .61f, .32f);
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            if (MicrophoneIcon || SendIcon)
            {
                ...
                if(MicrophoneIcon)
                {
                    ...
                    Arc(vh,c,unit*.41f,0,360,unit*.11f,new Color(1,.1f,.5f,recording?.20f:.09f));
            ...
            if (Nameplate)
            {
                var fill = new Color(.86f, .74f, .50f, .98f);

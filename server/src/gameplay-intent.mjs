...
The request identifies the original trigger. Its attributed history may now include later user evidence received while interpretation was pending. Reconsider the complete current request, including continuations, deliberate corrections, or cancellations; the original typed message may also appear in history and is one submission, not two requests. Later arrival or an overlapping timestamp alone does not mean cancellation. Never decide from an isolated keyword or silence gap. Preserve the original intent when new context merely completes or continues it; use the corrected intent when clearly replaced; return no_action when clearly canceled without a replacement. If the current intent remains ambiguous or incomplete, ask for clarification rather than executing the old proposal. Assistant fragments alone never authorize a player action.

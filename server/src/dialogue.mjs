...
const traits = {
  ...
  theo: "You are Theo, a charming married socialite at the club with a romantic partner who is not your spouse. You know your own situation; do not volunteer it to a stranger. No confrontation has happened. You can be defensive if the player brings up exposure. You can offer to step further into the existing VIP seating to talk privately, using invite_vip. This is an invitation: say 'We can talk in VIP, if you like' rather than claiming anyone has moved. The player must accept the invitation using the visible follow control. Never promise a different lounge, new room or offscreen destination. Once in VIP, talk quietly there and do not invite them again.",
};
export function characterInstructions(input) {
  ...
  if(input.character==='theo')localState.inVip=input.state.inVip??false;
...
export function validateInput(body) {
  ...
  return {character:body.character,message:body.message.trim(),state:Object.fromEntries(['loop','intimate','waiting','privateApproach','lucaPrepared','recognized','resolved','inVip'].map(k=>[k,k==='inVip'?s[k]===true:s[k]])),history:history.map(x=>({role:x.role,content:x.content}))};
}

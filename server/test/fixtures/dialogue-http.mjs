...
const server=createDialogueServer({apiKey:'TEST_ONLY_NOT_A_REAL_KEY',fetchImpl:async(_url,args)=>{
 ...
 const actions=message.startsWith('VIP_TEST')?['invite_vip']:message==='DELAY_TEST'?['wait']:message.startsWith('Wait right')?['wait','private_approach']:message.startsWith('Theo may')?['prepare_intervention']:message.startsWith('Could you')?['music_intimate']:['follow'];

...
- Music: team-supplied Suno outputs from Jetha, edited into loops; provenance and outstanding usage-term confirmation in [Team Music](licenses/Team-Music.md). Normal-change backspin: original procedural generation in tools/audio/generate_placeholders.py. On the local audio-polish branch, rewind uses musicvision31’s Record Scratch #1 under CC0 1.0; source and conversion details are retained in [the notice](licenses/RecordScratch-CC0.md). The local audio-polish branch includes these music replacements.

    ...
    public static class RenDesignerMouthImport
    {
        ...
        public static SkinnedMeshRenderer[] Add(RenDesignerEyeReview review, string handoffPath)
        {
            ...
            catch (Exception error)
            {
                ...
                audit.status = "FAIL_BINDING_ROLLED_BACK"; audit.exception = error.ToString(); audit.meshes = work.Select(w => w.Audit).ToArray(); throw;
            }
            finally { File.WriteAllText(AuditPath, JsonUtility.ToJson(audit, true)); Debug.Log("REN_DESIGNER_MOUTH_IMPORT_AUDIT: " + AuditPath + " " + audit.status); }
        }

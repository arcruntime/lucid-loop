...
namespace LucidLoop.CharacterArt.Editor
{
    ...
    [Serializable] public sealed class RenTokonMaterialInput
    {
        ...
        public RenTokonTextureInput baseOverride, shadow, control, closedBase, closedShadow, revisedControl, capControl;
    ...
    public static class RenTokonReviewBuilder
    {
        ...
        public static void BuildWindowsViewer()
        {
            ...
            foreach (var input in config.materials)
                foreach (var texture in new[] { input.baseOverride,input.shadow,input.control,input.closedBase,input.closedShadow,input.revisedControl,input.capControl }) if (texture != null && !string.IsNullOrEmpty(texture.asset)) ImportTexture(texture);
            ...
            var materialPairs = new Dictionary<string, (Material baseline,Material tokon,bool closed,Texture2D revisedControl,float revisedFaceMode,Texture2D capControl)>(StringComparer.Ordinal);
            foreach (var input in config.materials)
            {
                ...
                var capControl = input.capControl == null || string.IsNullOrEmpty(input.capControl.asset) ? null : AssetDatabase.LoadAssetAtPath<Texture2D>(input.capControl.asset);
                materialPairs.Add(input.sourceMaterialAsset,(baseline,saved,closed,revisedControl,input.revisedFaceMode,capControl));
            }
            ...
            foreach (var renderer in controller.ModelRoot.GetComponentsInChildren<Renderer>(true))
            {
                ...
                for (var slot = 0; slot < materials.Length; slot++)
                {
                    ...
                    materials[slot] = pair.baseline; slots.Add(new RenTokonReviewSlot { renderer = renderer,slot = slot,tokon = pair.tokon,closedEndpoint = pair.closed,revisedControl = pair.revisedControl,revisedFaceMode = pair.revisedFaceMode,capControl = pair.capControl });
                }
        ...
        static void ImportTexture(RenTokonTextureInput input)
        {
            ...
            var changed = importer.sRGBTexture != input.sRGB || importer.textureCompression != TextureImporterCompression.Uncompressed || importer.maxTextureSize != 8192 || importer.alphaIsTransparency;
            importer.sRGBTexture = input.sRGB; importer.textureCompression = TextureImporterCompression.Uncompressed; importer.maxTextureSize = 8192; importer.alphaIsTransparency = false;
